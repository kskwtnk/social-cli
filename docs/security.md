...
**注意**: このドキュメントの大部分は、将来のPhase 2以降（キーチェーン統合）を見据えた内容です。MVP（Phase 1）では `.env` ファイルを使用するため、まずは[MVP版セキュリティガイド](#mvp版セキュリティガイド-env使用)をご覧ください。

---

## 目次

- [MVP版セキュリティガイド (.env使用)](#mvp版セキュリティガイド-env使用) ← **Phase 1はこちら**
- [Phase 2以降: 認証情報管理（キーチェーン）](#phase-2以降認証情報管理キーチェーン)
...
Phase 1 MVPでは、シンプルさを優先し `.env` ファイルで認証情報を管理します。
...
**重要**:
- 通常のパスワードではなく、**App Password** を使用
- App Passwordは Settings → App Passwords から生成
...
# 3. App Passwordを即座に無効化
# Bluesky Settings → App Passwords → 該当パスワードを削除

# 4. 新しいApp Passwordを生成して .env を更新
```

### Phase 2への移行

将来的にセキュリティを強化したい場合は、OSキーチェーンへの移行を検討してください。詳細は [Phase 2以降: 認証情報管理（キーチェーン）](#phase-2以降認証情報管理キーチェーン) を参照。

---

## Phase 2以降：認証情報管理（キーチェーン）

**注意**: 以下のセクションはPhase 2以降の参考情報です。MVPでは実装しません。
